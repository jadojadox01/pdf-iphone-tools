...
export async function mergePdfs(files, options = {}, onStatus) {
  ...
  report(onStatus, 6, "Preparing merged PDF…");
  const merged = await PDFDocument.create();

  for (let index = 0; index < list.length; index += 1) {
    const file = list[index];
    report(onStatus, span(10, 88, index, list.length), `Adding ${file.name} (${index + 1} of ${list.length})…`);
    await yieldUi();
  ...
  report(onStatus, 94, "Saving merged PDF…");
