...
export async function compressPdf(file, options = {}, onStatus) {
  ...
  let blob;
  if (level.raster) {
    report(onStatus, 8, "Rebuilding pages as compressed images…");
    blob = await rasterizePdf(file, options.password, level, onStatus);
  } else {
    report(onStatus, 20, "Optimizing PDF structure…");
    const pdf = await loadPdfDocument(file, options.password);
    pdf.setTitle(pdf.getTitle() || "");
    pdf.setProducer(process.env.NEXT_PUBLIC_SITE_NAME || "PDFFlow");
    ...
    report(onStatus, 90, "Saving compressed PDF…");
  }
...
async function rasterizePdf(file, password, level, onStatus) {
  ...
  for (let pageNumber = 1; pageNumber <= pdfjsDoc.numPages; pageNumber += 1) {
    report(onStatus, span(12, 92, pageNumber - 1, pdfjsDoc.numPages), `Compressing page ${pageNumber} of ${pdfjsDoc.numPages}…`);
    await yieldUi();
