...
export async function persistMediaFile(id, bytes, mimeType, filename) {
  ...
  if (runningOnVercel()) {
    try {
      await putMediaBlob(mediaBlobPath(id, ext), bytes, mimeType);
      return `/media/${id}`;
    } catch {
...
function mediaResponse(bytes, media, mimeType) {
  const filename = String(media.filename || "image").replace(/"/g, "");
  const type = sniffImageMime(media.filename, mimeType || media.mimeType, bytes) || media.mimeType || "application/octet-stream";
...
export async function serveMedia(id) {
  ...
  if (media.data && media.data.length) {
    return mediaResponse(media.data, media);
  }

  const ext = mediaFileExtension(media.mimeType, media.filename);
  const fromBlob =
    (await getMediaBlob(mediaBlobPath(media.id, ext))) ||
    (await getMediaBlob(mediaBlobPath(media.id, ".png"))) ||
    (await getMediaBlob(mediaBlobPath(media.id, ".jpg"))) ||
    (await getMediaBlob(mediaBlobPath(media.id, ".jpeg"))) ||
    (await getMediaBlob(mediaBlobPath(media.id, ".webp")));
  if (fromBlob?.bytes?.length) {
    return mediaResponse(fromBlob.bytes, media, fromBlob.contentType);
  }

  const url = String(media.url || "").trim();
  if (url && /^https?:\/\//i.test(url) && /blob\.vercel-storage\.com/i.test(url)) {
    const remote = await getMediaBlob(url);
    if (remote?.bytes?.length) return mediaResponse(remote.bytes, media, remote.contentType);
  }
