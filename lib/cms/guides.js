...
import { extractSearchText, parseBlocks, planningFromBlocks, withPlanningBlock } from "@/lib/cms/blocks";
...
export async function getPublicGuidesForTool(toolSlug, { take = 4 } = {}) {
  ...
  return guides.filter((guide) => isPublicStatus(guide, now));
}
...
export function serializeGuide(guide, { preview = false } = {}) {
  ...
  return {
    ...
    relatedTools: (guide.relatedTools || [])
      ...
      .filter(Boolean),
    relatedGuides,
    primaryToolSlug: blocks.find((block) => block.type === "toolCta")?.data?.toolSlug || "",
...
export function buildGuideRecord(input, existing = {}) {
  ...
  const blocks = ensureRelationBlocks(withPlanningBlock(incoming, planning), input);
  ...
  const unpublished = ["draft", "unpublished", "archived"].includes(status);
  return {
    title,
    slug,
    ...
    robots: unpublished ? "noindex,follow" : input.robots || "index,follow",
