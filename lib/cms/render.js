...
import { buildGuideLinks, tokenizeLinks } from "./linkify";
import { getTool } from "@/lib/tools";

export function GuideBody({ doc, title, guide, className = "article-body" }) {
  if (!doc) return null;
  const ids = new Map();
  const catalog = buildGuideLinks(guide);
  const content = prepareArticleNodes(doc.content || [], title);
  const nodes = renderNodes(content, ids, catalog);
...
function renderNodes(nodes, ids, catalog) {
  return (nodes || []).map((node, index) => renderNode(node, index, ids, catalog));
}

function applyMarks(content, marks = []) {
  return marks.reduce((inner, mark) => {
    ...
    if (mark.type === "code") return <code>{inner}</code>;
    return inner;
  }, content);
}

function renderLink(href, content, key) {
  const safe = safeHref(href);
  if (!safe) return <span key={key}>{content}</span>;
  if (safe.startsWith("/")) {
    return (
      <Link key={key} href={safe} className="guide-backlink">
        {content}
      </Link>
    );
  }
  return (
    <a key={key} href={safe} className="guide-extlink" rel="noopener noreferrer">
      {content}
    </a>
  );
}

function renderNode(node, index, ids, catalog) {
  if (!node) return null;
  if (node.type === "text") {
    ...
    if (linkMark) {
      return renderLink(linkMark.attrs?.href, applyMarks(node.text || "", otherMarks), index);
    }
    const tokens = tokenizeLinks(node.text || "", catalog);
    if (tokens.length === 1 && !tokens[0].href) {
      return <span key={index}>{applyMarks(node.text || "", otherMarks)}</span>;
    }
    return (
      <span key={index}>
        {tokens.map((token, tokenIndex) => {
          const inner = applyMarks(token.text, otherMarks);
          if (!token.href) return <span key={tokenIndex}>{inner}</span>;
          return renderLink(token.href, inner, tokenIndex);
        })}
      </span>
    );
  }

  const children = renderNodes(node.content, ids, catalog);
  switch (node.type) {
    case "paragraph":
