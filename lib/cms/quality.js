import { extractHeadings, extractSearchText, getArticleDoc, parseBlocks } from "@/lib/cms/blocks";
import { DEVICE_CLONE_TITLE_RE, FAKE_DEVICE_CLAIM_RE } from "@/lib/cms/content-strategy";
...
export function assessGuideQuality(guide, meta = {}) {
  ...
  const hasToolLink =
    ...
    articleHasToolLink(getArticleDoc(blocks));
  ...
  if (guide.deviceId && planning.deviceIntent === "any") {
    warnings.push(item("device", "The page is tagged to a device hub, but planning says any device. Leave the hub empty unless this guide is truly for one verified device."));
  }

  const claimText = [guide.title, excerpt, guide.seoTitle, guide.seoDescription, extractSearchText(blocks)].join(" ");
  if (FAKE_DEVICE_CLAIM_RE.test(claimText)) {
    warnings.push(
      item(
        "device",
        "This draft claims iPhone testing. Do not write “tested on iPhone”, “works perfectly on iPhone”, or “tested on iPhone Safari” unless you verified it on a real iPhone.",
      ),
    );
  }
  if (planning.deviceIntent === "iphone" || planning.deviceIntent === "mac") {
    warnings.push(
      item(
        "device",
        `Device intent is ${planning.deviceIntent}. The owner currently has a Windows laptop and an Android phone. Keep this only if those steps were actually tested on that device.`,
      ),
    );
  }
  if (DEVICE_CLONE_TITLE_RE.test(guide.title || "") && planning.toolDependency && planning.toolDependency !== "none") {
    warnings.push(
      item(
        "device",
        "This title is a device clone of a tool task. Cover iPhone/Android/Windows as sections inside the PDFFlow tool guide instead of publishing another near-duplicate URL.",
      ),
    );
  }
...
function articleHasToolLink(node) {
  if (!node) return false;
  if (node.type === "toolCta" && getTool(node.attrs?.toolSlug)) return true;
  if (node.type === "buttonLink" && String(node.attrs?.href || "").includes("/tools/")) return true;
  if (
    node.type === "text" &&
    (node.marks || []).some((mark) => mark.type === "link" && String(mark.attrs?.href || "").includes("/tools/"))
  ) {
    return true;
  }
  return (node.content || []).some(articleHasToolLink);
}
...
function emptyBlocks(blocks) {
  return blocks.filter((block) => {
    if (["divider", "toc", "relatedTools", "relatedGuides", "planning", "articleDoc"].includes(block.type)) return false;
    const data = block.data || {};
    const text = [
      data.title,
      data.summary,
