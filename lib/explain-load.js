import { getPublicGuide, getPublicGuidesForTool, serializeGuide, serializeGuideCard } from "@/lib/cms/guides";
...
export async function loadRelatedGuides(tool, explain) {
  ...
  return linked
    .sort((a, b) => Number(isPrimaryForTool(b, tool.slug)) - Number(isPrimaryForTool(a, tool.slug)))
    .map((guide) => ({
      ...serializeGuideCard(guide),
      primary: isPrimaryForTool(guide, tool.slug),
    }));
}
